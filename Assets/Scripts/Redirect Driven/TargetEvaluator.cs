...
public class TargetEvaluator : MonoBehaviour
{
    ...
    public EnemyAngle EnemyInMoveDirection(Transform origin, float rayLength, Vector2 moveDirection)
    {
        ...
        for (var i = 0; i <= _searchResolution; i++)
        {
            ...
            var enemy = hit.collider.GetComponent<EnemyAngle>();
    ...
    public EnemyAngle GetParallelEnemyFromTargetLocation(Vector2 targetLocation, float rayLength, Vector2 direction)
    {
    ...
    public bool DotProductSuccess(EnemyAngle enemyAngle, Vector2 moveDirection)
    {
        return Vector2.Dot(enemyAngle.KillVector, moveDirection) > _dotKillThreshold;
    }
}